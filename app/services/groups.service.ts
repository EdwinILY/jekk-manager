...
import { Budget, BudgetContribution, NewBudget, UserContribution } from "../../models/budget.interface";
...
export const uploadAttachment = async (fileUri: string): Promise<string> => {
  ...
  const { data, error } = await supabase.storage.from("attachments").upload(filePath, decode(fileBase64), { contentType });
...
export const voteOnBudgetWithContribution = async (budgetId: number, userId: number, vote: "approve" | "reject", comment?: string, contributionAmount?: number): Promise<void> => {
  const { error } = await supabase.rpc("vote_on_budget_with_contribution", {
    p_budget_id: budgetId,
    p_user_id: userId,
    p_vote: vote,
    p_comment: comment || null,
    p_contribution_amount: contributionAmount || 0,
  });

  if (error) {
    console.error("Error voting on budget with contribution:", error);
    throw new Error("No se pudo registrar el voto: " + error.message);
  }
};
...
export const getUserVote = async (budgetId: number, userId: number): Promise<"approve" | "reject" | null> => {
  const { data, error } = await supabase.from("budget_votes").select("vote").eq("budget_id", budgetId).eq("user_id", userId).single();
...
export const isUserAdmin = async (groupId: number, userId: number): Promise<boolean> => {
  const { data, error } = await supabase.from("group_members").select("role").eq("group_id", groupId).eq("user_id", userId).single();
...
export const getBudgetAttachments = async (budgetId: number): Promise<{ url: string }[]> => {
  const { data, error } = await supabase.from("budget_attachments").select("url").eq("budget_id", budgetId).eq("type", "image");
...
export const getUserPendingContributions = async (userId: number): Promise<UserContribution[]> => {
  const { data, error } = await supabase.rpc("get_user_pending_contributions", {
    p_user_id: userId,
  });

  if (error) {
    console.error("Error getting user pending contributions:", error);
    throw new Error("No se pudieron obtener las contribuciones pendientes: " + error.message);
  }

  return data as UserContribution[];
};

export const getBudgetContributionSummary = async (
  budgetId: number
): Promise<{
  total_amount: number;
  ...
  const { data, error } = await supabase.rpc("get_budget_contribution_summary", {
    p_budget_id: budgetId,
  });

  if (error) {
    console.error("Error getting budget contribution summary:", error);
    throw new Error("No se pudo obtener el resumen de contribuciones: " + error.message);
  }

  return data?.[0] || null;
};

export const getBudgetContributions = async (budgetId: number): Promise<BudgetContribution[]> => {
  const { data, error } = await supabase.rpc("get_budget_contributions", {
    p_budget_id: budgetId,
  });

  if (error) {
    console.error("Error getting budget contributions:", error);
    throw new Error("No se pudieron obtener las contribuciones del presupuesto: " + error.message);
  }

  return data as BudgetContribution[];
};

export const getUserVoteWithContribution = async (
  budgetId: number,
  userId: number
): Promise<{
  ...
  const { data, error } = await supabase.from("budget_votes").select("vote, contribution_amount").eq("budget_id", budgetId).eq("user_id", userId).single();

  if (error && error.code !== "PGRST116") {
    // PGRST116 = no rows returned
    console.error("Error getting user vote with contribution:", error);
    throw new Error("No se pudo obtener el voto del usuario: " + error.message);
  }
